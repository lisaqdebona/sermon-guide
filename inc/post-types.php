...
function js_custom_init() {
    $post_types = array(
        array(
            'post_type' => 'sermons',
            'menu_name' => 'Sermon Guides',
            'plural'    => 'Sermon Guides',
            'single'    => 'Sermon Guide',
...
function set_custom_cpt_columns($columns) {
    ...
    if($post_type=='sermons') {
        unset( $columns['date'] );
        $columns['title'] = __( 'Name', 'idahograce' );
        $columns['visibility'] = __( 'Visibility', 'idahograce' );
        $columns['date'] = __( 'Date', 'idahograce' );
    }
...
function custom_post_column( $column, $post_id ) {
    ...
    if($post_type=='sermons') {
        switch ( $column ) {
            case 'visibility' :
                $visibility = get_field("sermon_visibility",$post_id);
                $val = ($visibility) ? ucwords($visibility) : 'Off';
                if($visibility=='on') {
                    $val = '<span alt="f155" class="dashicons dashicons-star-filled" style="color:#eabb07;font-size:25px;"></span>';
                }
                echo '<span class="visibility '.$visibility.'">'.$val.'</span>';
                break;
        }
    }
